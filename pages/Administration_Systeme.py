...
st.markdown("---")
st.caption("Mettez à jour l’application en 1 clic quand une nouvelle release est créée sur GitHub")

col1, col2 = st.columns([3, 1])

with col1:
    if st.button("🔄 Vérifier la dernière release", type="primary", use_container_width=True):
        with st.spinner("Connexion à GitHub..."):
            try:
                ...
                if r.status_code == 200:
                    latest = r.json()
                    st.success(f"✅ Dernière release : **{latest['tag_name']}** - {latest['name']}")
                    st.info(latest['body'] or "Aucune description")
                else:
...
with col2:
    if st.button("🚀 Mettre à jour maintenant depuis la release", type="secondary", use_container_width=True):
        with st.spinner("Mise à jour en cours..."):
            import subprocess
            try:
                result = subprocess.run(["git", "pull", "origin", "main"], capture_output=True, text=True, cwd="/opt/logistique-pro")
                if result.returncode == 0:
                    st.success("✅ Mise à jour terminée ! Redémarrage de Streamlit...")
                    subprocess.run(["pkill", "-f", "streamlit"], check=False)
                    subprocess.Popen(["nohup", ".venv/bin/streamlit", "run", "main.py", "--server.port", "8501", "--server.address", "0.0.0.0", ">", "streamlit.log", "2>&1", "&"], cwd="/opt/logistique-pro")
                    st.rerun()
                else:
                    st.error(f"Erreur git : {result.stderr}")
            except Exception as e:
                st.error(f"Erreur : {e}")

# ==================== 🚀 MISE À JOUR AUTOMATIQUE (VERSION STABLE) ====================
st.markdown("---")
    ...
    if st.button("🚀 Mettre à jour maintenant depuis la release", type="secondary", use_container_width=True):
        with st.spinner("Mise à jour en cours..."):
            ...
            try:
                ...
                if result.returncode == 0:
                    st.success("✅ Mise à jour terminée ! Redémarrage...")
                    subprocess.run(["pkill", "-f", "streamlit"], check=False)
                    st.rerun()
                else:
...
# ==================== 🚀 MISE À JOUR AUTOMATIQUE (VERSION FINALE) ====================
st.markdown("---")
st.caption("Mettez à jour l’application en 1 clic dès qu’une nouvelle release est créée sur GitHub")
...
with col1:
    if st.button("🔄 Vérifier la dernière release", type="primary", use_container_width=True, key="check_release"):
        with st.spinner("Connexion à GitHub..."):
            try:
                import requests
                r = requests.get("https://api.github.com/repos/XAV59213/logistiques/releases/latest", timeout=10)
...
with col2:
    if st.button("🚀 Mettre à jour maintenant depuis la release", type="secondary", use_container_width=True, key="update_release"):
        with st.spinner("Mise à jour en cours..."):
            import subprocess
            try:
                result = subprocess.run(["git", "pull", "origin", "main"], cwd="/opt/logistique-pro", capture_output=True, text=True, timeout=30)
...
with col1:
    if st.button("🔄 Vérifier la dernière release", type="primary", use_container_width=True, key="check_release_vfinal"):
        with st.spinner("Connexion à GitHub..."):
            try:
                import requests
                r = requests.get("https://api.github.com/repos/XAV59213/logistiques/releases/latest", timeout=10)
...
with col2:
    if st.button("🚀 Mettre à jour maintenant depuis la release", type="secondary", use_container_width=True, key="update_release_vfinal"):
        with st.spinner("Mise à jour en cours..."):
            import subprocess
            try:
                result = subprocess.run(["git", "pull", "origin", "main"], cwd="/opt/logistique-pro", capture_output=True, text=True, timeout=30)
