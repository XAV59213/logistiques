...
BASE_DIR = Path("/opt/logistique-pro/Logistique_Pro")
...
with col2:
    if st.button("🚀 Mettre à jour maintenant depuis la release", type="secondary", use_container_width=True, key="update_release_final"):
        with st.spinner("Mise à jour en cours..."):
            import subprocess
            try:
                result = subprocess.run(["git", "pull", "origin", "main"], cwd="/opt/logistique-pro/Logistique_Pro", capture_output=True, text=True, timeout=30)
                if result.returncode == 0:
                    st.success("✅ Mise à jour terminée ! Redémarrage en cours...")
                    # Ligne neutralisée par correctif page grise :
                    # subprocess.run(["pkill", "-f", "streamlit"], check=False)
                    st.warning("Mise à jour faite. Redémarre le service manuellement : systemctl restart logistique.service")
                    # Ligne neutralisée par correctif page grise :
                    # subprocess.Popen(["nohup", ".venv/bin/streamlit", "run", "main.py", "--server.port", "8501", "--server.address", "0.0.0.0", ">", "streamlit.log", "2>&1", "&"], cwd="/opt/logistique-pro/Logistique_Pro", shell=True)
                    st.warning("Mise à jour faite. Redémarre le service manuellement : systemctl restart logistique.service")
                    # st.rerun() neutralisé après mise à jour GitHub
                else:
