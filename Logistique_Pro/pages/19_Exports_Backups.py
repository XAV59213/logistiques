            ...
            try:
                ...
                if df_stock.empty:
                    ...
                    st.download_button(
                        ...
                        mime="text/csv",
                        use_container_width=True,
                        key="download_stock_items_csv",
                    )
            except Exception as e:
                st.error(f"Erreur export stock : {e}")

        st.divider()
        st.subheader("Sauvegarde de la base de données")
            ...
            for backup_file in backup_files:
                with st.container(border=True):
                    ...
                    with c1:
                        ...
                        st.caption(f"Modifié le : {datetime.fromtimestamp(backup_file.stat().st_mtime).strftime('%d/%m/%Y %H:%M:%S')}")
